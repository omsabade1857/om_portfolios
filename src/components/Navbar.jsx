...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center w-full h-20 text-white fixed bg-black px-4'>
        <div>
            <h1 className='text-5xl font-signature ml-2'>Omkar</h1>
        </div>
        <ul className='hidden md:flex'>
            {links.map(({id,link,title})=> (
                <li 
                key = {id} 
                class="cursor-pointer px-8 font-medium text-gray-500 hover:scale-105 duration-200">
        ...
        {links.map(({id,link,title})=> (
                <li 
                key = {id} 
                class="px-4 cursor-pointer capitalize py-6">
